...
TEST (wallet, import)
{
	...
	wallet->import.importFromFile (path.string ().c_str (), "");
...
TEST (wallet, change_seed)
{
	...
	ASSERT_EQ (2, wallet->accounts.getModel ().count ());
	wallet->import.importSeed (seed.data.to_string ().c_str ());
	ASSERT_EQ (1, wallet->accounts.getModel ().count ());
	rai::raw_key seed2;
	...
	wallet->import.importSeed (seed3.data.to_string ().c_str ());
	ASSERT_EQ (key1, wallet->account);
...
TEST (wallet, import_locked)
{
	...
	wallet->import.importSeed (seed1.data.to_string ().c_str ());
	rai::raw_key seed2;
	system.wallet (0)->store.seed (seed2, rai::transaction (system.wallet (0)->store.environment, nullptr, false));
